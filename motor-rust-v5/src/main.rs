...
fn main() {
    println!("--- 🚀 Motor Rust V5.2: Entrenamiento Optimizado (f32) ---");
    let seq_len = 128;
    let num_filters = 8;
    let kernel_size = 5;

    let (x_raw, y_raw) = generate_sensor_data(2000, seq_len);
    ...
    for epoch in 1..=50 {
        ...
        if epoch % 10 == 0 {
            println!("Época {:3} | Loss: {:.6}", epoch, total_loss / 2000.0);
        }
    }

    println!("\n⏱️ Entrenamiento completado en: {:?}", start.elapsed());
    let model = ModelV5 { conv, dense };
    let f = File::create("modelo_v5.json").unwrap();
    serde_json::to_writer(f, &model).unwrap();
    println!("--- Modelo f32 Guardado Exitosamente ---");
}
